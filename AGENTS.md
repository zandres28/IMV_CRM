...
- **Workflow activo:** `IMV - Soportes de Pago (Gemini)` id `H2YRscVbeB19GaQ6` (webhook `soporte-pago`, 18 nodos). Webhook Chatwoot (inbox 5, cuenta 1) → `If -PRUEBAS` (solo `+573006143054`) → dedup por `messageId` → Gemini OCR → `Consultar Deuda en CRM` → registrar/responder.
...
- **Silencio para imágenes no-comprobante sin contexto de pago (2026-09-17, H2YRscVbeB19GaQ6):** antes, Toda imagen clasificada por Gemini como no-comprobante disparaba la respuesta automática "ola, recibimos tu imagen. 📎...". Ahora: la rama **false** de `¿Es comprobante válido?` alimenta un **nuevo IF `¿Cliente sugiere pago?`** (id `e7d42d1a-97c0-406c-81b7-0a9331491f4f`) que evalúa `{{ $json.textSuggestsPayment }}` (keywords: pago/soporte/comprobante/transferencia/nequi/bancolombia/daviplata/recibo/consignacion). Rama true → `Responder: No Parece Comprobante`; rama false → **silencio** (el agente revisa en Chatwoot). Además: (1) `Preparar Llamada a Gemini` ahora PROPAGA `textSuggestsPayment` en su salida (antes quedaba perdido — ese nodo reconstruye su JSON desde cero); (2) `¿Es comprobante válido?` ahora exige 3 condiciones AND: `esComprobante=true` + `monto>0` + `confianzaAlta=true` (confianza ≥ 0.7). Lección: al insertar un IF entre ramas, verificar que el campo usado (textSuggestsPayment) sobreviva el camino completo; modificar vía `n8n_update_partial_workflow` con `validateOnly` primero y `branch="true"/"false"` (un `sourceIndex` desincronizado rompe la lógica de ramas).
